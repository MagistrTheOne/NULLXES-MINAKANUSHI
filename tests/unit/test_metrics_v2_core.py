...
def test_wait_frame_counterfactual_is_move_to(tmp_path: Path) -> None:
    cfg = load_config(
        ROOT / "configs" / "architecture" / "cpu_dev.yaml",
        training_path=ROOT / "configs" / "training" / "stage0_overfit.yaml",
        runtime_path=ROOT / "configs" / "runtime" / "cpu.yaml",
    ...
    cfg = replace(
        cfg,
        training=replace(cfg.training, steps=1, eval_every=1, checkpoint_every=1, log_every=1, sequence_length=6),
    )
    trainer = Trainer(cfg, ROOT)
    pkt = trainer.unroll(1, scenario="const_velocity", episode_index=0)
    assert pkt.candidates[0].objective == "WAIT"
    assert pkt.candidates[1].objective == "MOVE_TO"
    assert pkt.candidates[0].strategy_id != pkt.candidates[1].strategy_id
    assert not torch.allclose(pkt.pred_future, pkt.alt_future)
    metrics = trainer._metrics(pkt)
    assert metrics["counterfactual_quality"] > 0.0
    assert metrics["future_diversity"] >= 0.0
