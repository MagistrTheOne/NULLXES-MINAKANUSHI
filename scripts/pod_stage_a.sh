#!/usr/bin/env bash
# Stage A bring-up on RTX PRO 6000 BW. gpu_train_v01 only. Never construct 6.8B.
set -euo pipefail
...
if [ ! -d "${REPO}/.git" ]; then
  rm -rf "${REPO}"
  git clone --branch MINAKANUSHI-v0.1-foundation https://github.com/MagistrTheOne/NULLXES-MINAKANUSHI.git "${REPO}"
fi
cd "${REPO}"
mkdir -p "${OUT}"
git fetch --tags --force
git checkout MINAKANUSHI-v0.1-foundation
...
# Training YAML is not on the foundation tag yet — write it on the pod.
...
echo "=== PROBE gpu_train_v01 (not 6.8B) ==="
python - <<'PY'
import json, time
...
ident = manifest["architecture"] == "MINAKANUSHI"
report = {
